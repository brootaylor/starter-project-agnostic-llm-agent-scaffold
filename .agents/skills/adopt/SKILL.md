...
- **`blueprint/build-plan.md`** - the ordered feature list as a checklist. **Mark
  ...
  immediately. Remember the roadmap is not the work queue: a build-plan line only
  becomes buildable once a spec for it exists in `docs/features/` or `docs/specs/`
  and a human has marked it `**Status:** Ready`.
