...
5. **Planning readiness**
   ...
   - Count checked and unchecked leaf items in `blueprint/build-plan.md` when it
     has real content. Report it as roadmap context, not as progress.
...
7. **Current workflow state**
   ...
   - Flag active spec on `main`, all spec steps checked but no completion, or a
     ...
     for the spec type. For a feature, also flag a work order whose `Spec:` line
     is missing or points at a file that no longer exists, and any spec marked
     `Complete` with no implementation, or still `Ready` after its work merged.
     For a rollback, confirm its target spec is `Complete` and do not compare it
     to the queue.
   - Report the spec queue itself: counts by `Draft` / `Ready` / `Complete` across
     `docs/features/` and `docs/specs/`, skipping `_component-template.spec.md`.
     A `Ready` feature spec whose dependency specs are still `Draft` is a blocker
     worth naming, since `/feature` will refuse it.
   - Treat `blueprint/build-plan.md` as an optional roadmap. A build-plan checkbox
     disagreeing with a spec `**Status:**` line is drift; the spec wins.
8. **Git**
