...
- repeat with the next `Ready` spec
...
1. Resume an active work order in `blueprint/context/current-feature.md`.
2. Otherwise select the next spec marked `**Status:** Ready` in `docs/features/`,
   then `docs/specs/`. Use `blueprint/build-plan.md` ordering only to break a tie
   between several `Ready` specs.
3. Continue until no `Ready` spec remains or `continuous.maxFeatures` completed
   features have been counted.

**The status gate is absolute here.** Never build a `Draft` spec and never
re-implement a `Complete` one. Never promote a spec to `Ready` to give yourself
more work: that is a human decision, and taking it would defeat the gate the
project is built around. When the queue empties because everything left is
`Draft`, **stop and report**, naming the specs that need human review. Running out
of `Ready` specs is a normal, successful end to a Continuous run, not a failure to
route around.

`resume` explicitly resumes the active work order. A spec name may set the
starting item only when no different work item is active.
...
- current spec queue by status and the next `Ready` spec
