...
- No argument: resume the current feature if one exists, otherwise target the next
  `Ready` spec in `docs/features/`, then `docs/specs/`. Never a `Draft` spec:
  stop and report which specs need human review instead. Never re-implement a
  `Complete` one. Promoting a spec to `Ready` is a human act and is not available
  to this mode.
- A name or path: target that spec, for example `/autopilot button` or
