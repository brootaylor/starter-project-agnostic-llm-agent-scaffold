---
name: overview
description: "Validate and, when needed, normalize the two planning docs before generating blueprint/context/project-overview.md from blueprint/project-plan.md and blueprint/build-plan.md. The overview is the AI-facing product context that project instructions load every session; docs/project-brief.md remains authoritative for stack and conventions, and each spec's Status line remains the work queue. Use when the user runs /overview, invokes $overview, has just finished writing or editing the plans, asks to shape rough plans into the Blueprint format, or asks to regenerate the project overview."
---

# overview - turn the two plans into AI-facing product context
...
- **One source of truth, and know which one.** Merge both plans into one coherent
  document. After this runs, the AI reads the overview rather than the raw plans.
  The overview is the **product** source of truth: problem, users, features, data
  model, build order, deployment.
  It is **not** the authority on conventions or stack. `docs/project-brief.md`
  holds those, and it wins on any disagreement about stack selection, coding
  conventions, browser targets, accessibility standard, or agent behaviour rules.
  Do not restate those sections in the overview; point at `docs/project-brief.md`
  instead, so there is one place to change them.
  It is also **not** the work queue. The `**Status:**` line on each spec in
  `docs/features/` and `docs/specs/` is what `/feature` and `/status` read.
  The overview is optional: a project whose specs are written can build without
  one.
