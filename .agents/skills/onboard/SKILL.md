...
**If `docs/project-brief.md` exists, it is the source of truth for conventions,
not this file.** Do not copy its stack selections, naming rules, browser targets,
accessibility standard, or agent rules into `coding-standards.md`; point at it
instead, so there is one place to change them. Fill in only what the build loop
adds on top. Likewise, if `docs/specs/` and `docs/features/` exist, the
`**Status:**` line on each spec is the work queue - say so in your hand-off and
do not set up `build-plan.md` as a competing tracker.
